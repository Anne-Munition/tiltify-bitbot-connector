...
async function getCampaign() {
  return axios
    .get(`https://tiltify.com/api/v3/campaigns/${process.env.TILTIFY_CAMPAIGN_ID}`, {
      headers: {
...
async function getRewards() {
  return axios
    .get(`https://tiltify.com/api/v3/campaigns/${process.env.TILTIFY_CAMPAIGN_ID}/rewards`, {
      headers: {
        Authorization: `Bearer ${process.env.TILTIFY_ACCESS_TOKEN}`,
      },
    })
...
function storeRewards(rewards) {
  ...
  fs.writeFileSync('./rewards.json', JSON.stringify(rewardObj, null, 2), {
    encoding: 'utf8',
  });
}
